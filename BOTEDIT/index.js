...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `saiu tarde ja @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'calmae menor to fazendo',
				success: 'acabei ja',
				error: {
					stick: 'faz de novo que deu errado :/',
					Iv: 'seila'
				},
				only: {
					group: 'opa , comando privado',
					ownerG: 'so pode adm man',
					ownerB: 'pera la amigao , só meu criador pode fazer isso',
					admin: 'pera la amigao , só meu criador pode fazer isso',
					Badmin: 'me da adm pra funcionar :) ''

o lwal
			}

			const botNumber = client.user.jid
			const ownerNumber = ["55799636675@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'yt2mp3':
					if (args.length < 1) return reply('onde tá o link?')
				...
				case 'ytsearch':
					if (args.length < 1) return reply('quem voce tá procurando?')
				...
				case 'stalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'onde ta o nome do usuário?', text, {quoted: mek})
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('oque deseja escrever? ')
					teks = body.slice(7)
					reply(mess.wait)
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('onde ta o link ? ')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('onden tá o texto?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
...
//NAO FUNCIONA MAIS	
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('membro novo :)')
						welkom.push(from)
